...
void drawTime(bool RTC_TYPE){
  ...
  if(timePage == TIME_H_M_S){
    ...
    // 写入日期
    matrix.setCursor(12, 5);
    int displayMonth = timeinfo.tm_mon;
    String month = displayMonth < 10 ? "0" : "";
    month += displayMonth;
  ...
  for (byte band = 0; band < NUM_BANDS; band++) {
    ...
    switch (rhythmPage){
      case RHYTHM_MODEL1:
        ...
        matrix.drawFastVLine((band), (MATRIX_SIDE - barHeight), barHeight, hsv2rgb(color,100,100));
        color += 360 / (NUM_BANDS + 4);
        // 绘制顶点
        //matrix.drawPixel((MATRIX_WIDTH - 1 - band), MATRIX_SIDE - peak[band] - 1, matrix.Color(150,150,150));
        matrix.drawPixel((band), MATRIX_SIDE - peak[band] - 1, matrix.Color(150,150,150));
        break;  
      case RHYTHM_MODEL2:
        // 绘制条形
        ...
        for (int y = MATRIX_SIDE; y >= MATRIX_SIDE - barHeight; y--) {
          ...
          matrix.drawPixel((band), y, matrix.Color(r, g, b));
      ...
      case RHYTHM_MODEL3:
        ...
        for (int y = MATRIX_SIDE; y >= MATRIX_SIDE - barHeight; y--) {
          ...
          matrix.drawPixel((band), y, hsv2rgb(y * (255 / MATRIX_WIDTH / 5) + colorTime, 100, 100));
        }
        break;
      case RHYTHM_MODEL4:
        // 此模式下，只绘制顶点      
        ...
        matrix.drawPixel((band), (MATRIX_SIDE - peak[band] - 1), 
        matrix.Color(model4ColorArrar[peak[band]][0], model4ColorArrar[peak[band]][1], model4ColorArrar[peak[band]][2]));
        break;  
      default:
        break;
